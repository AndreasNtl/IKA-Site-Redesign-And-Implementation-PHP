...
if (isset($_POST['save'])){
   ...
   $email0 =          mysqli_real_escape_string($con, $_POST['email0']);
   ...
   $amka =          mysqli_real_escape_string($con, $_POST['amka']);
	 ...
	 if(!filter_var($email0,FILTER_VALIDATE_EMAIL)) {
		$display =  true;
		$email0error = false;
		$email0_error = "Η διεύθυνση που εισάγατε στον εργοδότη είναι λανθασμένη";
	 }else {
		$email0error = true;
	}
	if (strlen($ame) != 10) {
		$display = true;
		$ameerror = false;
		$ame_error = "Ένας ΑΜΕ αποτελείται απο 10 ψηφία";
	}else {
		$ameerror = true;
	}
    ...
    if(!filter_var($email,FILTER_VALIDATE_EMAIL)) {
		$display =  true;
		$emailerror = false;
		$email_error ="Η διεύθυνση που εισάγατε στον υπάλληλο είναι λανθασμένη";
	}else {
		$emailerror = true;
	}
	if (strlen($amka) != 11) {
	...
	if($display){
		$display = false;
		 ...
		 $display = true;
		$result = mysqli_query($con, "SELECT * FROM employer WHERE email = '" . $email0 . "' and AME = '" . $ame . "'");
		$result1 = mysqli_query($con, "SELECT * FROM employee WHERE email = '" . $email . "' and amka = '" . $amka . "'");
		 if($row = mysqli_fetch_array($result)) {
			 $name0 = $row['name'];
	    	 $surname0 = $row['surname'];
			 $birthdate0 = $row['birthdate'];
		     $ame0 = $row['AME'];
		 }else {
		    $display = false;
			$errormsg0 = "Ο εργοδότης δεν βρέθηκε στη βάση";
	     }
...
<html lang="en">
  <head>
    ...
    <title>IKA-ETAM Βεβαίωση απασχόλησης προσωπικού(φορολογική χρήση)</title>
				...
				<ul class="nav navbar-nav">
					...
					<li class="dropdown">
						...
						<ul class="dropdown-content">
							<li><a href="./emp-vevaiwsh-apasxolishs-proswpikou.php"> <b>Βεβαίωση απασχόλησης προσωπικού(φορολογική χρήση)</b> </a></li>
		...
		<div  id="printableArea" class="container" style="background-color:white;">
		  ...
		  <div class="table-responsive-vertical">
				  ...
				  <?php if (!$display) { ?>
						<thead>
						<tr>
						  <th>Email εργοδότη</th>
						  <th>ΑΜΕ</th>
						</tr>
					  </thead>
					  <tbody>
							<tr>
							  <td data-title="Email εργοδότη"><input required=""  name="email0" value="<?php if($error) echo $email0; ?>"  type="text"> </input> </td>
							   <td data-title="ΑΜΕ"><input required=""  name="ame" value="<?php if($error) echo $ame; ?>" type="text"> </input>  </td>
							</tr>
					  </tbody>
					  <?php } else { ?>
					  <thead>
					   ...
					   <tbody>
							<tr>
							   <td data-title="Όνομα"> <?php  echo $name0; ?> </td>
							   <td data-title="Επώνυμο"> <?php  echo $surname0; ?> </td>
							   <td data-title="Ημ. Γέννησης"> <?php echo $birthdate0; ?>  </td>
							   <td data-title="Email εργοδότη"> <?php  echo $email0; ?> </td>
							   <td data-title="ΑΜΕ"> <?php echo $ame; ?>  </td>
							</tr>
					  </tbody>
				      <?php } ?>
				 </table>
				 ...
				 <table id="table" class="table table-hover">
				  ...
				  <div class="bebaiwsh bebaiwsh-keimeno">
						...
						<br>
						<p><strong>Βεβαιώνουμε ότι:&nbsp;&nbsp;&nbsp;&nbsp;</strong>Ο/Η Ασφαλισμένος/η με τα άνωθεν ατομικά στοιχεία απασχολείται στην επιχείρηση του άνωθεν αναφερόμενου εργοδότη.
						<br>
						<br></br>
						<p><strong>Υπογραφή:&nbsp;&nbsp;&nbsp;&nbsp;</strong>o εργοδότης<p>
						<br></br>
						...
						<span class="text-danger"><?php if (isset($email0_error)) echo $name0_error; ?></span> <br>
						<span class="text-danger"><?php if (isset($ame_error)) echo $ame_error; ?></span> <br>
						...
						<span class="text-danger"><?php if (isset($errormsg0)) echo $errormsg0; ?></span> <br>
