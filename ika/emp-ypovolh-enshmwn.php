...
if (isset($_POST['submit'])) {
	...
	$apasxolhsh =       mysqli_real_escape_string($con, $_POST['apasxolhsh']);
	...
	if (!$error) {
		if (isset($_SESSION['usr_id']) && ($_SESSION['usr_role'] == "employer" )){
		    $id = $_SESSION['usr_id'];
			$name = $_SESSION['usr_name'];
			$email = $_SESSION['usr_email'];
			$ame = $_SESSION['usr_ame'];
			echo "*";
			echo $ame;
			echo "*";
		    if(mysqli_query($con, "INSERT INTO employer_applications(name, AME, email, AMKA, apasxolhsh, dateFrom, dateTo, standBy, approval, employer_id ) VALUES('" . $name . "', '" . $ame . "', '" . $email . "', '" . $amka . "', '" . $apasxolhsh . "', '" . $dateFrom . "', '" . $dateTo . "', 'false', 'false', '" . $id . "')")) {
			...
			$ame =          mysqli_real_escape_string($con, $_POST['ame']);
			$id = 1;
			...
			if(   !   ($row = mysqli_fetch_array($result))   ) {
				...
				if(   !   ( mysqli_query($con, "INSERT INTO employer(id,AME,name,surname,birthdate,branch_store,address,email,password) VALUES('1','0000000000','no','profile','00-00-0000','no_store','no_address','noprofile@yahoo.com','" . md5("00000000") . "')") )   ){
					$error=1;
				}
			}
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if (strlen($ame) != 10) {
				$error = true;
				$ame_error = "Ένας ΑΜE αποτελείται απο 10 ψηφία";
			}
			
		    if(!$error && mysqli_query($con, "INSERT INTO employer_applications(name, AME, email, AMKA, apasxolhsh, dateFrom, dateTo, standBy, approval, employer_id ) VALUES('" . $name . "', '" . $ame . "', '" . $email . "', '" . $amka . "', '" . $apasxolhsh . "', '" . $dateFrom . "', '" . $dateTo . "', 'false', 'false', '" . $id . "')")) {
...
<html lang="en">
  <head>
    ...
    <title>IKA-ETAM Υποβολή ενσήμων εργαζομένου</title>
				...
				<ul class="nav navbar-nav">
					...
					<li class="dropdown">
						...
						<ul class="dropdown-content">
							...
							<li><a href="./emp-ypovolh-enshmwn.php"> <b>Υποβολή ενσήμων εργαζομένου</b> </a></li>
...
<!-- Text input-->
			<div class="form-group">
				<label class="col-md-4 control-label"  >AMΕ</label>   
				<div class="col-md-4">
					<?php if (isset($_SESSION['employer'])) { ?>
					<input id="ame" name="ame" type="text" value="<?php  echo $ame; ?>" class="form-control input-md" disabled>	
					<?php } else { ?>
					<input id="ame" name="ame" type="text" value="<?php if (isset($ame)) echo $ame; ?>" placeholder="Συμπληρώστε τον ΑΜΕ σας(10 ψηφία)" class="form-control input-md" required="">
					<span class="text-danger"><?php if (isset($ame_error)) echo $ame_error; ?></span>
					<?php } ?>
				</div>
			</div>
			<?php } ?>
			...
			<legend>Βήμα 2 : Προσδιορίστε το ΑΜΚΑ του εργαζομένου σας,και τον τύπο απασχόλησής του<br></legend>

			<div class="form-group">
				<label class="col-md-4 control-label">AMKA εργαζομένου</label>   
				<div class="col-md-4">
					<input id="amka" name="amka" type="text" value="<?php if (isset($amka)) echo $amka; ?>" placeholder="AMKA του εργαζομένου σας(11 ψηφία)" class="form-control input-md" required="">
			...
			<div class="form-group">
			   ...
			   <div  class="form-control" style="border-style: none;"  >
					<select  name="apasxolhsh" style="margin-left:5%" >
						<option value="merikh apasxolhsh">μερική απασχόληση</option>
						<option value="olikh apasxolhsh">ολική απασχόληση</option>
					</select>
               </div>				
			</div>	
			...
			<br></br>			<br></br>
			<legend>Βήμα 3 : Προσδιορίστε το διάστημα για το οποίο θέλετε να υποβάλλετε τα ένσημα του εργαζομένου σας<br></legend>
